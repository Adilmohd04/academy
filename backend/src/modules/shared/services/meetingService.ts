...
export const getMeetingRequestById = async (id: string): Promise<MeetingRequest | null> => {
  // Fetch meeting request WITHOUT FK join (no FK constraints exist in this database)
  const { data, error } = await supabase
    .from('meeting_requests')
    .select('*')
    .eq('id', id)
  ...
  // Manually fetch time_slot data if needed
  if (data && data.time_slot_id) {
    const { data: timeSlot } = await supabase
      ...
      .single();
    if (timeSlot) {
      (data as any).time_slot = timeSlot;
    }
  }
 ...
 */
export const getScheduledMeetingById = async (id: string): Promise<any | null> => {
  // Try fetching from meeting_bookings first as it seems to be the primary table now
  // Fetch WITHOUT joins first to ensure we get the record if it exists
  ...
  if (bookingError && bookingError.code !== 'PGRST116') {
    console.error('Error fetching meeting booking:', bookingError);
  }

  if (booking) {
    // Fetch related data manually to be robust
    let timeSlot = null;
    ...
    if (booking.teacher_id) {
      const { data: teacher } = await supabase
        .from('profiles')
        .select('full_name')
        .eq('clerk_user_id', booking.teacher_id)
        .single();
    ...
    return {
      ...
      topic: teacherSlot?.topic,
      description: teacherSlot?.description,
      teacher_name: teacherName
    };
  }

  // Fallback to scheduled_meetings (legacy?)
  const { data, error } = await supabase
    .from('scheduled_meetings')
    .select('*')
    .eq('id', id)
...
export const getStudentUpcomingMeetings = async (studentId: string): Promise<any[]> => {
  try {
    ...
    const { data: bookings, error: bookingsError } = await supabase
      ...
      .in('status', ['paid', 'approved'])
      // Use yesterday to account for timezone differences (client might be behind UTC)
      .gte('meeting_date', new Date(Date.now() - 86400000).toISOString().split('T')[0])
 ...
 * Returns all bookings with status info for box display
 */
export const getPendingMeetingsForAdmin = async (): Promise<any[]> => {
  // Get ALL meeting bookings (pending, approved) for box system
  // Fetch WITHOUT FK joins (no FK constraints exist in this database)
  const { data, error } = await supabase
    .from('meeting_bookings')
    .select('id, student_name, student_email, student_phone, meeting_date, time_slot_id, teacher_slot_id, notes, payment_amount, payment_status, approval_status, created_at, updated_at, status')
  ...
  // Manually fetch teacher_slot_availability data
  const slotIds = [...new Set(data.map((b: any) => b.teacher_slot_id).filter(Boolean))];
  let slotsMap: Record<string, any> = {};
  if (slotIds.length > 0) {
    const { data: slots } = await supabase
      .from('teacher_slot_availability')
      .select('id, teacher_id, date, topic, description, is_free, time_slot_id, max_capacity, current_bookings, is_unlimited, deadline_utc')
      .in('id', slotIds);
    if (slots) {
  ...
  const timeSlotIds = [...new Set([
    ...data.map((b: any) => b.time_slot_id),
    ...Object.values(slotsMap).map((s: any) => s.time_slot_id)
  ].filter(Boolean))];
  ...
  if (teacherIds.length > 0) {
    ...
    const { data: teachersByClerk } = await supabase
      .from('profiles')
      .select('id, clerk_user_id, full_name, email')
      .in('clerk_user_id', teacherIds);
    if (teachersByClerk) {
      teachersByClerk.forEach((t: any) => { teachersMap[t.clerk_user_id] = t; });
    }
    // Also try by UUID id
    const { data: teachersById } = await supabase
      .from('profiles')
      .select('id, clerk_user_id, full_name, email')
      .in('id', teacherIds);
    if (teachersById) {
      teachersById.forEach((t: any) => { teachersMap[t.id] = t; });
    }
  }

  // Transform the data to include teacher info, capacity, and deadline
  const transformedData = data.map((booking: any) => {
    const teacherSlot = slotsMap[booking.teacher_slot_id] || null;
    const teacher = teacherSlot ? (teachersMap[teacherSlot.teacher_id] || null) : null;
    const timeSlot = teacherSlot ? (timeSlotsMap[teacherSlot.time_slot_id] || null) : null;
    
    // Parse deadline_utc into date and time components
    const deadlineUtc = teacherSlot?.deadline_utc;
    let deadlineDate = null;
    let deadlineTime = null;
    if (deadlineUtc) {
      const deadline = new Date(deadlineUtc);
      deadlineDate = deadline.toISOString().split('T')[0];
      deadlineTime = deadline.toTimeString().split(' ')[0];
    }
    ...
    return {
      ...
      is_free: teacherSlot?.is_free || false,
      // Slot capacity info
      max_capacity: teacherSlot?.max_capacity || 10,
      current_bookings: teacherSlot?.current_bookings || 0,
      is_unlimited: teacherSlot?.is_unlimited || false,
      // Deadline info (parsed from deadline_utc)
 ...
 * Supports filtering by student_id or teacher_id
 */
export const getPastMeetings = async (filters: {
  studentId?: string;
  teacherId?: string;
  limit?: number;
}): Promise<any[]> => {
  // Fetch bookings WITHOUT FK joins (no FK constraints exist)
  let query = supabase
    .from('meeting_bookings')
    .select('*')
    .in('meeting_status', ['completed', 'cancelled'])
  ...
  // Manually fetch related profiles and slots
  return await enrichMeetingBookings(data);
};

/**
 * Get upcoming meetings (upcoming or ongoing)
 * Supports filtering by student_id or teacher_id
 */
...
export const getMeetingsByStatus = async (
  ...
  let query = supabase
    ...
    .order('meeting_date', { ascending: status === 'upcoming' || status === 'ongoing' });
  ...
  if (error) {
    console.error(`Error fetching ${status} meetings:`, error);
    throw error;
  }
...
/**
 * Helper: Enrich meeting bookings with profile and slot data (replaces FK joins)
 */
async function enrichMeetingBookings(bookings: any[]): Promise<any[]> {
  // Collect unique student_ids and teacher_ids
  ...
  const slotIds = [...new Set(bookings.map(b => b.teacher_slot_id).filter(Boolean))];

  // Fetch profiles for students and teachers
  const allProfileIds = [...new Set([...studentIds, ...teacherIds])];
  let profilesMap: Record<string, any> = {};
  if (allProfileIds.length > 0) {
    // Try fetching by clerk_user_id first (meeting_bookings may store clerk_user_id)
    const { data: profilesByClerk } = await supabase
      .from('profiles')
      .select('id, clerk_user_id, full_name, email')
      .in('clerk_user_id', allProfileIds);
    if (profilesByClerk) {
      profilesByClerk.forEach(p => { profilesMap[p.clerk_user_id] = p; });
    }
    // Also try by UUID id
    const { data: profilesById } = await supabase
      ...
      .in('id', allProfileIds);
    if (profilesById) {
      profilesById.forEach(p => { profilesMap[p.id] = p; });
    }
  }
  ...
  if (slotIds.length > 0) {
    const { data: slots } = await supabase
      .from('teacher_slot_availability')
      .select('id, topic, description, is_free')
      .in('id', slotIds);
    if (slots) {
  ...
  return bookings.map(b => ({
    ...b,
    student: profilesMap[b.student_id] || null,
    teacher: profilesMap[b.teacher_id] || null,
    slot: b.teacher_slot_id ? slotsMap[b.teacher_slot_id] || null : null,
  }));
}
