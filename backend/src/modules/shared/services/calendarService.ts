...
export async function createMeetEvent(args: CreateMeetEventArgs): Promise<CreateMeetEventResult> {
  ...
  if (!auth) {
    console.error('❌ Google Calendar auth failed - check GOOGLE_SERVICE_ACCOUNT_JSON and GOOGLE_IMPERSONATED_USER');
    throw new Error('Google Calendar is not configured');
  }

  console.log('🔑 Google Calendar auth successful');
  ...
  const insertRes = await calendar.events.insert({
    ...
    requestBody: {
      ...
      attendees: (args.attendeesEmails || []).map(email => ({ 
        email,
        responseStatus: 'needsAction' // Attendees will receive invite
      })),
      conferenceData: {
        createRequest: {
          requestId,
      ...
      guestsCanInviteOthers: true, // Let attendees invite others if needed
      guestsCanModify: false, // Don't let them modify the event details
      guestsCanSeeOtherGuests: true, // Let them see who else is invited
      // Set to 'public' to avoid "ask to join" - anyone with link can join
      visibility: 'default',
      reminders: {
        useDefault: false,
        overrides: [
