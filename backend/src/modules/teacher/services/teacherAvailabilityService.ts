...
export class TeacherAvailabilityService {
  ...
  async saveSlotAvailability(
    ...
    try {
      // First, get the teacher's profile to check their free status
      ...
      for (const slot of slots) {
        // If teacher is FREE, all slots MUST be free (ignore individual slot setting)
        // If teacher is PAID, use the individual slot's is_free setting
        const slotIsFree = isTeacherFree ? true : (slot.is_free || false);

        // Compute deadline_utc from booking_deadline_date + booking_deadline_time
        let deadlineUtc: string | null = null;
        if (slot.bookingDeadlineDate && slot.bookingDeadlineTime) {
          deadlineUtc = new Date(`${slot.bookingDeadlineDate}T${slot.bookingDeadlineTime}:00`).toISOString();
        } else if (slot.bookingDeadlineDate) {
          // Default to 18:00 if no time specified
          deadlineUtc = new Date(`${slot.bookingDeadlineDate}T18:00:00`).toISOString();
        }

        const { data, error } = await supabase
          .from('teacher_slot_availability')
          .upsert({
            teacher_id: teacherId, // Use clerk_user_id, not profile.id
            date: slot.date,
            ...
            is_available: true,
            is_free: slotIsFree, // Use calculated free status
  ...
  async getAvailableSlotsForTeacher(teacherId: string): Promise<any[]> {
    try {
      ...
      // Get teacher pricing to determine if teacher offers free sessions
      const { data: teacherPricing } = await supabase
        .from('teacher_pricing')
        .select('is_free, price_per_meeting')
        .eq('teacher_id', teacherId)
        .single();
      
      const isTeacherFree = teacherPricing?.is_free === true;
      const teacherPrice = teacherPricing?.price_per_meeting || null;
      
      console.log('💰 Teacher pricing:', { isTeacherFree, teacherPrice });
      ...
      const formattedSlots = availableSlots.map((slot: any) => {
        // Determine final is_free status:
        // If teacher is FREE teacher, ALL slots are free regardless of slot setting
        // Otherwise use slot's is_free value
        const finalIsFree = isTeacherFree ? true : (slot.is_free === true);
        
        // Determine meeting price:
        // If free, price is 0
        // Otherwise use slot custom_price, then teacher price, then slot meeting_price
        const finalPrice = finalIsFree ? 0 : (slot.custom_price || teacherPrice || slot.meeting_price || null);
        ...
        return {
          ...
          // Use calculated values that respect teacher pricing
          is_free: finalIsFree,
          meeting_price: finalPrice,
          topic: slot.topic,
