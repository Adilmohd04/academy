...
export class TeacherAvailabilityController {
   ...
   */
  async saveWeeklyAvailability(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async saveSlotAvailability(req: Request, res: Response): Promise<void> {
    try {
      const teacherId = req.auth?.userId;
      if (!teacherId) {
        res.status(401).json({ error: 'Unauthorized' });
  ...
  async getWeeklyAvailability(req: Request, res: Response): Promise<void> {
    try {
      console.log('🔍 getWeeklyAvailability called - weekStartDate:', req.params.weekStartDate);
      const teacherId = req.auth?.userId;
  ...
  async getSlotAvailability(req: Request, res: Response): Promise<void> {
    try {
      console.log('🔍 getSlotAvailability called - query:', req.query);
      const teacherId = req.auth?.userId;
      console.log('🔍 Teacher ID:', teacherId);
      if (!teacherId) {
        res.status(401).json({ error: 'Unauthorized' });
   ...
   */
  async getAvailableSlotsForStudent(req: Request, res: Response): Promise<void> {
    try {
   ...
   * Check slot capacity
   */
  async checkSlotCapacity(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async getTeachersWithAvailability(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async deleteSlotAvailability(req: Request, res: Response): Promise<void> {
    try {
      const teacherId = req.auth?.userId;
      if (!teacherId) {
        res.status(401).json({ error: 'Unauthorized' });
   ...
   */
  async getAvailableSlotsForTeacher(req: Request, res: Response): Promise<void> {
    try {
      const { teacherId } = req.params;
